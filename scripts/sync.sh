...
ROLES_FILE="$SRC_DIR.gitconfig.roles"
...
migrate_legacy_config() {
  ...
  if [[ -n "$author_name" && -n "$personal_email" ]]; then
    ...
    if [[ -f "$ROLES_FILE" ]]; then
      sed -i.bak "s/^GIT_PERSONAL_NAME=.*/GIT_PERSONAL_NAME=\"$author_name\"/" "$ROLES_FILE"
      sed -i.bak "s/^GIT_PERSONAL_EMAIL=.*/GIT_PERSONAL_EMAIL=\"$personal_email\"/" "$ROLES_FILE"

      if [[ -n "$work_email" ]]; then
        sed -i.bak "s/^GIT_WORK_NAME=.*/GIT_WORK_NAME=\"$author_name\"/" "$ROLES_FILE"
        sed -i.bak "s/^GIT_WORK_EMAIL=.*/GIT_WORK_EMAIL=\"$work_email\"/" "$ROLES_FILE"
      fi
    ...
    cat > "$temp_file" <<'EOF'
# =============================================================================
# NOTICE: Git configuration has been migrated to src/.gitconfig.roles
# =============================================================================
# The git config logic below is DEPRECATED and no longer used.
# Your Git identities are now managed by:
#   - src/.gitconfig.roles in your dotfiles repo
...
#   1. Edit src/.gitconfig.roles in your dotfiles repo
...
generate_git_configs() {
  ...
  if [[ ! -f "$roles_file" ]]; then
    warning "Roles file not found at $roles_file"
    return 0
  fi
  ...
  info "Generating Git configuration files from $roles_file..."

  # Source the roles configuration
  if ! source "$roles_file" 2>/dev/null; then
    error "Failed to load $roles_file - syntax error detected"
  ...
  if [[ -z "${GIT_PERSONAL_NAME:-}" || -z "${GIT_PERSONAL_EMAIL:-}" ]]; then
    error "GIT_PERSONAL_NAME and GIT_PERSONAL_EMAIL must be set in $roles_file"
    return 1
  fi

  if [[ -z "${GIT_WORK_NAME:-}" || -z "${GIT_WORK_EMAIL:-}" ]]; then
    error "GIT_WORK_NAME and GIT_WORK_EMAIL must be set in $roles_file"
    return 1
  fi
...
# To update your personal identity, edit src/.gitconfig.roles in your dotfiles repo and run sync.sh
...
# This file is automatically generated by sync.sh from .gitconfig.roles
# Do not edit directly - changes will be overwritten
# To update your work identity, edit src/.gitconfig.roles in your dotfiles repo and run sync.sh
...
display_git_status() {
  ...
  echo "  To customize identities, edit: $ROLES_FILE"
  echo ""
}
...
if check_git_version; then
  ...
  if generate_git_configs; then
    ...
    if ! validate_git_config; then
      ...
      error "Please fix $ROLES_FILE and try again"
      exit 1
    fi
